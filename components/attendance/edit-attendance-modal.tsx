...
export function EditAttendanceModal({
  ...
  useEffect(() => {
    if (record) {
      ...
      const hasDailyRateOverride = record.override?.dailyRateOverride != null ||
                                    record.dailyRateOverride != null;
      const dailyRateValue = record.override?.dailyRateOverride ?? record.dailyRateOverride;
      ...
      setState({
        ...
        useDailyRateOverride: hasDailyRateOverride,
        dailyRateOverride: hasDailyRateOverride && dailyRateValue != null
          ? dailyRateValue.toString()
          : "",
      });
      setError(null);
    }
  ...
  const handleSave = async () => {
    ...
    // Determine daily rate override value
    const dailyRateOverrideValue = state.useDailyRateOverride
      ? parseFloat(state.dailyRateOverride) || null
      : null;
    ...
    await onSave({
      ...
      dailyRateOverride: dailyRateOverrideValue,
      reason: finalReason,
    ...
    >
      <div className="space-y-4">
        ...
        <div className="border-t border-gray-200 pt-4">
          <h4 className="text-sm font-medium text-gray-900 mb-2">
            Daily Rate Override
            <span className="text-xs font-normal text-gray-500 ml-2">(e.g., training rate for this day)</span>
          </h4>
          <div className="flex items-center gap-2 mb-2">
            <input
              type="checkbox"
              id="useDailyRateOverride"
              checked={state.useDailyRateOverride}
              onChange={(e) => setState({
                ...state,
                useDailyRateOverride: e.target.checked,
                dailyRateOverride: e.target.checked ? state.dailyRateOverride : "",
              })}
              className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <label htmlFor="useDailyRateOverride" className="text-sm text-gray-600">
              Override daily rate for this day
            </label>
          </div>
          {state.useDailyRateOverride && (
            <input
              type="number"
              step="0.01"
              min="0"
              value={state.dailyRateOverride}
              onChange={(e) => setState({ ...state, dailyRateOverride: e.target.value })}
              placeholder="e.g., 500.00"
              className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-blue-500 focus:border-blue-500"
            />
          )}
          <p className="text-xs text-gray-500 mt-1">
            Affects all calculations for this day: basic pay, OT, late/undertime deductions.
          </p>
        </div>
        ...
        <div className="border-t border-gray-200 pt-4">
          ...
          >
            ...
            <option value="Training rate adjustment">Training rate adjustment</option>
