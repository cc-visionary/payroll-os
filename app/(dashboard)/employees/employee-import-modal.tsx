...
// =============================================================================
// PeopleOS PH - Employee Import Modal
// =============================================================================
...
import {
  ...
  importEmployees,
  exportEmployees,
  type EmployeeImportValidationResult,
...
export function EmployeeImportModal({ isOpen, onClose }: EmployeeImportModalProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      // Validate
      startTransition(async () => {
        const result = await validateEmployeeImport(base64Content);
        setValidation(result);
        if (result.success || result.validEmployees.length > 0) {
          setStep("preview");
  ...
  const handleImport = async () => {
    ...
    setError(null);
    startTransition(async () => {
      const result = await importEmployees(fileContent);
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Import Employees" size="lg">
      {step === "upload" && (
        <>
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <p className="text-sm text-blue-700 mb-3">
                Download the Excel template with the correct column headers and fill in your employee
                data. The template includes reference sheets for departments, roles, and more.
              </p>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">
                Select your completed Excel file (.xlsx) to import employees.
              </p>
              <input
                ref={fileInputRef}
                type="file"
                ...
                className="hidden"
                id="employee-xlsx-upload"
              />
              <label
                htmlFor="employee-xlsx-upload"
      ...
      {step === "preview" && validation && (
        <>
          <div className="space-y-4">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <h4 className="font-medium text-green-800">
                Ready to import {validation.validEmployees.length} employee(s)
              </h4>
            </div>
            ...
            <div className="border rounded-lg overflow-auto max-h-64">
              <table className="min-w-max w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {validation.validEmployees.map((emp, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-4 py-2 text-sm text-gray-900">{emp.rowNumber}</td>
                      <td className="px-4 py-2 text-sm text-gray-900">{emp.name}</td>
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {emp.departmentName || "-"}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500">
                        {emp.roleScorecardTitle || "-"}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500">{emp.employmentType}</td>
                      <td className="px-4 py-2 text-sm text-gray-500">{emp.hireDate}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <ModalFooter>
            ...
            <Button onClick={handleImport} loading={isPending} disabled={validation.validEmployees.length === 0}>
              Import {validation.validEmployees.length} Employee(s)
            </Button>
          </ModalFooter>
        </>
      )}
